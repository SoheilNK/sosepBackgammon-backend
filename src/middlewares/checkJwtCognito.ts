...
export const checkJwtCognito = async (
  ...
  try {
    ...
    let access_tokenSections: string[];
    let x_id_token: string;
    if (
    ...
    if (!x_id_token) {
      ...
      const x_id_tokenJSON = Buffer.from(
        x_id_tokenSections[1],
        "base64"
      ).toString("utf8");
    ...
    const headerJSON = Buffer.from(access_tokenSections[0], "base64").toString(
      "utf8"
    );
